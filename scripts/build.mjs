...
<main id="contenido">
  <section id="inicio" class="hero"><div class="container">
    ...
    <div class="signal-strip" aria-label="${esc(l.summaryLabel)}"><div><strong>${projects.length}</strong><span>${esc(l.summaryCases)}</span></div><div><strong>1</strong><span>${esc(l.summaryCandidate)}</span></div><div><strong>ES / EN</strong><span>${esc(l.summaryLanguages)}</span></div><div><strong>855+</strong><span>${esc(l.summaryLimits)}</span></div></div>
  </div></section>
